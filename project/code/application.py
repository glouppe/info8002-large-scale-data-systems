...
def main(arguments):
    ...
    # Depending on how fast your blockchain is, this will return a proper result.
    print(storage.retrieve(key))
    # Using the callback object, you can also wait for the operation to be completed.
    callback.wait()
    # Now the key should be available, unless a different node `put` a new value.
    print(storage.retrieve(key))
